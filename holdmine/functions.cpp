...
int __stdcall Hook_Recv(SOCKET s, char * buf, int len, int flags)
{
	...
	if (HookPackets::instance()->toClient != nullptr)
	{
		HookPackets::instance()->toClient->onPacket(buf, len, flags);
	}
	return ret;
}
...
int __stdcall Hook_Send(SOCKET s, const char * buf, int len, int flags)
{
	...
	if (HookPackets::instance()->toServer != nullptr)
	{
		HookPackets::instance()->toServer->onPacket(buf, len, flags);
	}
	return ret;
}